...
export function HppListPage({ data }: { data: Context }) {
  ...
  return <><AdminPageHeader eyebrow="KALKULATOR PAKET UMRAH" title="HPP & Simulasi Paket" description="Hitung seluruh biaya paket, gratis Tour Leader (FOC), keuntungan kantor, dan harga jual secara otomatis." actions={[{ href: "/admin/manajemen/hpp-umroh/master-harga", label: "Master harga", secondary: true }, { href: "/admin/manajemen/hpp-umroh/baru", label: "Buat perhitungan baru" }]} />
    <section className="management-panel hpp-help"><div><span><Calculator /></span><div><strong>Petunjuk Praktis Menghitung Paket:</strong><p>1. Tekan tombol <b>“Buat perhitungan baru”</b> di atas.<br />2. Masukkan nama paket dan perkiraan jumlah jamaah (misal: 35 orang). Semua biaya lain sudah terisi otomatis.<br />3. Tentukan target keuntungan kantor & komisi agen di Langkah 4.<br />4. Klik <b>“Simpan draft”</b> atau <b>“Finalkan”</b>. Harga di website <u>tidak akan berubah</u> sebelum Anda menekan tombol “Terapkan”.</p></div></div></section>
    <section className="management-panel"><header><div><small>{active.length} PERHITUNGAN AKTIF</small><h2>Riwayat simulasi HPP</h2><p>Klik salah satu simulasi di bawah untuk melihat rincian biaya, download file Excel resmi, atau menerapkan harga ke website.</p></div></header>{active.length ? <div className="management-table-wrap"><table className="management-table"><thead><tr><th>Nama simulasi</th><th>Jumlah Jamaah</th><th>HPP / Modal per Pax</th><th>Harga Jual Jamaah</th><th>Status</th><th>Aksi</th></tr></thead><tbody>{active.map((item) => <tr key={item.id}><td><strong>{item.title}</strong><small>Diperbarui {date.format(item.updatedAt)}</small></td><td>{item.paxCount} orang</td><td>{rupiah.format(Number(item.hppPerPax))}</td><td><strong>{rupiah.format(Number(item.sellingPrice))}</strong></td><td><span className={`management-status ${item.status === "applied" ? "good" : item.status === "draft" ? "warn" : "neutral"}`}>{statusLabel(item.status)}</span></td><td><Link className="management-row-link" href={`/admin/manajemen/hpp-umroh/${item.id}`}>Buka rincian <ArrowRight /></Link></td></tr>)}</tbody></table></div> : <div className="management-empty"><Calculator /><p>Belum ada perhitungan HPP. Tekan “Buat perhitungan baru” di atas untuk memulai.</p></div>}</section>
...
export function HppCreatePage({ data }: { data: Context }) {
  return <><AdminPageHeader eyebrow="PERHITUNGAN BARU" title="Hitung HPP paket Umrah" description="Harga standar sudah disiapkan otomatis. Cukup ubah bagian yang berbeda untuk paket ini." backHref="/admin/manajemen/hpp-umroh" /><HppCalculator masters={data.masterPrices} departures={data.departures} /></>;
}
...
export function HppDetailPage({ data, costing }: { data: Context; costing: Context["costings"][number] }) {
  ...
  return <><AdminPageHeader eyebrow="HASIL PERHITUNGAN HPP" title={costing.title} description={`Formula ${costing.formulaVersion} · ${costing.paxCount} jamaah · ${costing.durationDays} hari`} backHref="/admin/manajemen/hpp-umroh" actions={costing.status === "draft" || costing.status === "final" ? [{ href: `/admin/manajemen/hpp-umroh/${costing.id}/edit`, label: "Edit angka perhitungan", icon: "edit" }] : undefined} />
    <section className="hpp-detail-hero"><div><small>HPP BERSIH / MODAL PER JAMAAH</small><strong>{rupiah.format(Number(costing.hppPerPax))}</strong><span>Biaya riil {rupiah.format(Number(costing.subtotalBase))} + Biaya gratis Tour Leader {rupiah.format(Number(costing.focTourLeader))}</span></div><div><small>HARGA JUAL REKOMENDASI</small><strong>{rupiah.format(Number(costing.sellingPrice))}</strong><span>Harga yang diterapkan: {rupiah.format(Number(costing.appliedPrice ?? costing.sellingPrice))}</span></div><span className={`management-status ${costing.status === "applied" ? "good" : "neutral"}`}>{statusLabel(costing.status)}</span></section>
    <div className="hpp-detail-columns"><section className="management-panel"><header><div><small>RINCIAN BIAYA</small><h2>Komponen biaya per jamaah</h2><p>Semua biaya di bawah sudah dikonversi ke Rupiah per orang.</p></div></header><div className="hpp-detail-groups">{grouped.map((group) => <div key={group.key}><h3>{group.label}</h3>{group.items.map((item) => <p key={item.id}><span><strong>{item.name}</strong><small>{item.currency} · {item.costBasis === "group" ? "dibagi rata seluruh jamaah" : item.costBasis === "room_per_night" ? "kamar per malam" : "per jamaah"}</small></span><b>{rupiah.format(Number(item.computedPerPax))}</b></p>)}</div>)}</div></section>
