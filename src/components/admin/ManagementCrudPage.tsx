...
export function ManagementDetailPage({ module, id, data }: { module: string; id: string; data: Context }) {
  ...
  if (module === "pembayaran") {
    ...
    return <><AdminPageHeader eyebrow="DETAIL PEMBAYARAN" title={rupiah(item.amount)} description="Pembayaran terhubung ke invoice dan kwitansi secara otomatis." backHref={backHref} /><Panel title="Informasi pembayaran"><DetailGrid rows={[["Invoice", invoice?.number ?? "—"], ["Booking", item.booking?.bookingNumber], ["Pembayar", item.booking?.payerName], ["Tanggal", <DateText key="date" value={item.paidAt} />], ["Metode", item.method], ["Referensi", item.reference], ["Status", item.status], ["Status Laporan", <span key="report-status" style={{ fontWeight: 600, color: isIncluded ? "#15803d" : "#64748b" }}>{isIncluded ? "Masuk ke Laporan & Kas" : "Dikecualikan dari Laporan (Data Testing)"}</span>], ["Kwitansi", receipt?.number ?? "Belum dibuat"]]} /><div className="management-payment-document-actions">{invoice ? <Link href={`/admin/manajemen/invoice-kwitansi/${invoice.id}`}><FileText /> Lihat invoice</Link> : null}{receipt ? <Link href={`/admin/manajemen/invoice-kwitansi/${receipt.id}`}><Download /> Lihat kwitansi</Link> : item.status === "confirmed" ? <EnsureReceiptButton bookingId={item.bookingId} paymentId={item.id} /> : null}</div></Panel><Panel title="Status perhitungan laporan" description="Gunakan fitur ini jika transaksi merupakan data uji coba/testing atau ingin dikecualikan dari angka kas dan rekap omzet tanpa menghapus riwayatnya."><p className="management-form-note" style={{ marginBottom: "8px" }}>Status saat ini: <strong>{isIncluded ? "Termasuk di dalam laporan & saldo kas" : "Dikecualikan dari laporan, saldo kas, dan piutang"}</strong>.</p><ReportInclusionToggleForm entity="payment" id={item.id} isIncluded={isIncluded} /></Panel>{item.status === "confirmed" ? <Panel title="Catat refund"><RefundForm payments={refundPayments} accounts={data.accounts.filter((row) => row.status === "active").map(({ id: accountId, name }) => ({ id: accountId, name }))} /></Panel> : null}<section className="management-danger-zone"><span><strong>Hapus data pembayaran</strong><small>Gunakan jika ada salah input angka pemasukan. Angka pemasukan kas, selisih kas, dan piutang otomatis dikalkulasi ulang.</small></span><DeleteButton id={item.id} name={`pembayaran ${rupiah(item.amount)}`} type="payment" variant="form" /></section></>;
  }
  if (module === "keuangan") {
    ...
    const isIncluded = transaction.isIncludedInReports !== false;
    return <><AdminPageHeader eyebrow="DETAIL TRANSAKSI" title={transaction.description} description="Transaksi disimpan sebagai catatan audit." backHref={backHref} /><Panel title="Informasi transaksi"><DetailGrid rows={[["Tanggal", <DateText key="date" value={transaction.transactionAt} />], ["Jenis", transaction.direction], ["Nominal", rupiah(transaction.amount)], ["Paket", data.packages.find((row) => row.id === transaction.packageId)?.name], ["Status koreksi", transaction.isReversal ? "Transaksi pembalik" : "Transaksi aktif"], ["Status Laporan", <span key="report-status" style={{ fontWeight: 600, color: isIncluded ? "#15803d" : "#64748b" }}>{isIncluded ? "Masuk ke Laporan & Kas" : "Dikecualikan dari Laporan (Data Testing)"}</span>]]} /></Panel>{!transaction.paymentId ? <Panel title="Status perhitungan laporan" description="Kecualikan transaksi pengeluaran/pemasukan ini dari perhitungan kas dan laba jika merupakan transaksi uji coba."><ReportInclusionToggleForm entity="cash_transaction" id={transaction.id} isIncluded={isIncluded} /></Panel> : <p className="management-form-note" style={{ margin: "16px" }}>Transaksi kas ini otomatis tersinkronisasi dengan data pembayaran induknya.</p>}</>;
  }
