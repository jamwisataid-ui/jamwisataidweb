...
export function ManagementDetailPage({ module, id, data }: { module: string; id: string; data: Context }) {
  ...
  if (module === "invoice-kwitansi") {
    ...
    return <><AdminPageHeader eyebrow="DETAIL DOKUMEN" title={item.number} description={item.kind === "invoice" ? `Status pembayaran: ${paymentLabel}. Kwitansi dibuat otomatis setiap pembayaran diterima.` : "Kwitansi ini dibuat otomatis dari pembayaran yang diterima."} backHref={backHref} action={item.kind === "invoice" && outstanding > 0 ? { href: `/admin/manajemen/pembayaran/baru?booking=${item.bookingId}`, label: "Catat pembayaran" } : undefined} /><Panel title={item.kind === "invoice" ? "Invoice" : "Kwitansi"}><DetailGrid rows={rows} /><div className="management-document-downloads"><a className="management-create-link" href={`/api/admin/management/issued-documents/${item.id}`}><Download /> Download PDF / Print</a><a className="management-create-link secondary" href={`/api/admin/management/issued-documents/${item.id}?format=png`}><Download /> Download PNG</a></div></Panel></>;
  }
  return null;
}
