      ...
      <div className="border-b border-[#0A1D3A]/8 bg-[#FFFDF8] shadow-[0_7px_24px_rgba(16,43,63,.07)]">
        <div className="jam-container flex h-[76px] items-center justify-between">
          ...
          <div className="flex items-center gap-2.5">
            <a href={whatsapp} target="_blank" rel="noopener noreferrer" className="hidden min-h-11 items-center gap-2 rounded-[14px] bg-[#D4AF37] px-5 text-xs font-extrabold text-white shadow-[0_8px_20px_rgba(20,125,115,.16)] transition hover:-translate-y-0.5 hover:bg-[#B8860B] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#D4AF37] sm:inline-flex">
              <MessageCircle className="size-4" aria-hidden="true" /> Konsultasi Umrah
            </a>
            <a href={whatsapp} aria-label="Konsultasi cepat via WhatsApp" target="_blank" rel="noopener noreferrer" className="grid size-11 place-items-center rounded-[13px] bg-[#D4AF37] text-white sm:hidden">
        ...
        <div className="fixed inset-x-0 top-[108px] bottom-0 z-50 bg-[#06152B]/45 backdrop-blur-[2px] xl:hidden" onMouseDown={(event) => event.target === event.currentTarget && closeMenu()}>
          <div id="mobile-navigation" ref={drawerRef} role="dialog" aria-modal="true" aria-label="Menu navigasi" className="ml-auto flex h-full w-[min(88vw,390px)] flex-col bg-[#FFFDF8] p-5 shadow-[-20px_0_50px_rgba(7,28,43,.18)]">
            ...
            <div className="mt-auto rounded-[18px] bg-[#F7F3E9] p-4 text-xs leading-5 text-[#59616D]">
              ...
              <a href={whatsapp} target="_blank" rel="noopener noreferrer" className="mt-4 flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#D4AF37] px-4 font-bold text-white"><MessageCircle className="size-4" /> Konsultasi Umrah</a>
            </div>
          </div>
        </div>
      ) : null}
