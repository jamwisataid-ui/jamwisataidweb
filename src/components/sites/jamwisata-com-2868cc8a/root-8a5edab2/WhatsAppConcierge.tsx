    ...
    <>
      ...
      >
        ...
        <span className="hidden items-center gap-2.5 text-left sm:flex">
          ...
          <span className="grid h-8 w-14 place-items-center rounded-lg bg-white px-1.5"><Image src={`${assetRoot}/logo.png`} alt="" width={500} height={184} className="h-auto w-full" /></span>
        </span>
      </button>
      ...
      <div className={`concierge-popover fixed right-3 bottom-[calc(max(1rem,env(safe-area-inset-bottom))+5rem)] z-[84] w-[calc(100%-1.5rem)] sm:right-6 sm:bottom-[104px] sm:w-[370px] ${open ? "concierge-popover-open" : "concierge-popover-closed"}`}>
          ...
          >
            <div className="sticky top-0 z-10 flex items-center gap-3 bg-gradient-navy p-5 text-white">
              <span className="grid h-11 w-20 shrink-0 place-items-center rounded-[13px] bg-white px-2"><Image src={`${assetRoot}/logo.png`} alt="Logo Jam Wisata" width={500} height={184} className="h-auto w-full" /></span>
            ...
            <div className="p-5 pb-[max(1.25rem,env(safe-area-inset-bottom))]">
              <p className="text-[13px] leading-6 text-[#59616D]">Assalamu’alaikum. Ada yang bisa kami bantu? Pilih topik konsultasi di bawah ini.</p>
              <div className="mt-4 space-y-2.5">
                {topics.map(({ icon: Icon, title, detail, message }) => (
                  <a key={title} href={whatsappHref(message, `Concierge — ${title}`)} target="_blank" rel="noopener noreferrer" className="group grid min-h-[70px] grid-cols-[42px_1fr_20px] items-center gap-3 rounded-[16px] border border-[#0A1D3A]/9 p-3 transition hover:-translate-y-0.5 hover:border-[#D4AF37]/55 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#D4AF37]">
                    <span className="grid size-10 place-items-center rounded-[12px] bg-[#F7F3E9] text-[#D4AF37]"><Icon className="size-4.5" aria-hidden="true" /></span>
                    <span className="min-w-0"><strong className="block text-[12px] leading-snug text-[#0A1D3A]">{title}</strong><span className="mt-1 block text-[10px] leading-4 text-[#68707A]">{detail}</span></span>
                    <ChevronRight className="size-4 text-[#9A9A9A] transition group-hover:translate-x-0.5 group-hover:text-[#D4AF37]" aria-hidden="true" />
                  </a>
                ))}
              </div>
              <p className="mt-4 text-center text-[10px] text-[#68707A]">Sen–Jum 09.00–17.00 · Sab 09.00–14.00 WIB</p>
            </div>
          </div>
        </div>
    </>
