      ...
      >
        <div className="jam-container">
          ...
          <div className="mt-10 grid gap-5 lg:grid-cols-[.41fr_.59fr]">
            <div className="group relative min-h-[390px] overflow-hidden rounded-[24px] bg-[#0A1D3A] sm:min-h-[460px] lg:min-h-0">
              <Image
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-8">
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-2 rounded-full border border-white/16 bg-white/12 px-3 py-2 text-[11px] font-bold backdrop-blur-sm"><BookOpenCheck className="size-4 text-[#D4AF37]" /> Manasik Terarah</span>
                  <span className="inline-flex items-center gap-2 rounded-full border border-white/16 bg-white/12 px-3 py-2 text-[11px] font-bold backdrop-blur-sm"><UsersRound className="size-4 text-[#D4AF37]" /> Tim Pendamping</span>
                </div>
              </div>
            </div>
            <div className="grid gap-4 min-[430px]:grid-cols-2">
              {facilityGroups.map(([Icon, title, items, surface]) => (
                <article key={title} className={`rounded-[20px] border border-[#0A1D3A]/8 p-5 transition duration-200 hover:-translate-y-0.5 hover:border-[#D4AF37]/55 sm:p-6 ${surface}`}>
                  <span className="grid size-11 place-items-center rounded-[12px] bg-[#0A1D3A] text-[#D4AF37] shadow-[0_8px_18px_rgba(10,29,58,.14)]"><Icon className="size-5" strokeWidth={1.7} /></span>
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {tours.map(([name, place, image]) => (
              <a
                ...
                className="group relative aspect-[16/11] overflow-hidden rounded-[20px] focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#D4AF37]"
              >
                <Image
                  src={`${assetRoot}/${image}`}
                ...
                <div className="absolute inset-x-0 bottom-0 p-6">
                  <p className="text-[11px] text-[#D4AF37]">{place}</p>
