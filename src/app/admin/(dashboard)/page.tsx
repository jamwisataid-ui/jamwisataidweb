...
export default async function DashboardPage() {
  ...
  return <>
    <section className="admin-welcome">
      <div><p><Sparkles aria-hidden /> Setiap Waktu Bernilai Ibadah</p><h1>Selamat datang di ruang kerja Jam Wisata.</h1><span>Kelola setiap informasi dengan teliti agar jamaah menerima panduan yang jelas dan dapat dipercaya.</span><div><Link className="admin-primary-button" href="/admin/paket/baru"><Plus aria-hidden />Tambah paket</Link><Link className="admin-text-button" href="/" target="_blank">Lihat website <ArrowRight aria-hidden /></Link></div></div>
    ...
    <section className="admin-section-heading"><div><p className="admin-eyebrow">RINGKASAN KONTEN</p><h2>Kondisi website saat ini</h2></div><span>Data langsung dari Neon</span></section>
    ...
    <section className="admin-dashboard-grid">
      <article className="admin-panel admin-quick-panel"><div className="admin-panel-heading"><div><p className="admin-eyebrow">AKSI CEPAT</p><h2>Apa yang ingin dikerjakan?</h2></div></div><div className="admin-quick-grid">{quickActions.map(([title, description, Icon, href]) => <Link href={href} key={title}><span><Icon /></span><div><strong>{title}</strong><p>{description}</p></div><ArrowRight /></Link>)}</div></article>
