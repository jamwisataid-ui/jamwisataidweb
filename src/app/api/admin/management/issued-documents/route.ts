...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json(await issueTransactionDocument({ kind, bookingId: body.bookingId, paymentId: body.paymentId, invoiceAmount: body.invoiceAmount, actorId: session.user.id }));
  } catch (error) {
