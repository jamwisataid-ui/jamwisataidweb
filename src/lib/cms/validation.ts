...
export const packageFormSchema = z.object({
  ...
  category: z.enum(["umrah", "hajj", "halal-tour"]),
  packageType: z.enum(["bintang-5", "plus", "reguler", "tour"]),
  badge: z.string().trim().optional(),
  summary: z.string().trim().optional(),
  imageUrl: z.string().trim().min(1, "Gambar wajib dipilih."),
  durationDays: z.coerce.number().int().min(1).max(60),
  detailUrl: z.string().trim().optional(),
  whatsappMessage: z.string().trim().min(10),
  seoTitle: z.string().trim().max(70).optional(),
  seoDescription: z.string().trim().max(170).optional(),
  featured: z.coerce.boolean().default(false),
  sortOrder: z.coerce.number().int().min(0).default(0),
  departureDate: z.iso.date(),
  departureLabel: z.string().trim().min(3),
  returnDate: z.union([z.iso.date(), z.literal("")]).optional(),
  manasikDate: z.union([z.iso.date(), z.literal("")]).optional(),
  airline: z.string().trim().min(2),
  departureAirport: z.string().trim().min(2),
  arrivalAirport: z.string().trim().optional(),
  price: z.coerce.number().int().positive(),
  capacity: z.union([z.coerce.number().int().positive(), z.literal("")]).optional(),
  availableSeats: z.union([z.coerce.number().int().min(0), z.literal("")]).optional(),
  departureStatus: z.enum(["open", "limited", "full", "closed", "coming-soon"]),
  makkahHotel: z.string().trim().min(2),
  makkahStar: z.coerce.number().int().min(1).max(5),
  makkahDistance: z.string().trim().optional(),
  madinahHotel: z.string().trim().min(2),
  madinahStar: z.coerce.number().int().min(1).max(5),
